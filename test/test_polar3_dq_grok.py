...
from polarALL_state_int16 import process_sequence_polar, polar_to_complex_tensor

def bit_order_conversion(state_vector, n_qubits):
    """
    将DeepQuantum的大端序(MSB first)转换为polarALL_state_int16的小端序(LSB first)
    ...
    Returns:
        转换为polarALL_state_int16格式的状态向量
    """
    converted = np.zeros_like(state_vector)

    for i in range(len(state_vector)):
        # 将索引i从MSB first转换为LSB first
        # 例如：对于3比特系统，DeepQuantum的索引2(10₂)对应polarALL_state_int16的索引1(01₂)
...
def compare_implementations_polar3_dq(n_qubits=4, a=30, b=0, c=0):
    """
    比较polarALL_state_int16和DeepQuantum的实现
    只输出简洁的误差信息格式，类似test_error_state3.py
    ...
    # 3. 运行polarALL_state_int16并获取历史
    from polarALL_state_int16 import process_sequence_polar  # Already imported at module level
    ...
    # 将polar初始状态转换为复数形式
    from polarALL_state_int16 import polar_to_complex_tensor  # Already imported at module level
    ...
    for step, gate_tuple in enumerate(seq):
        ...
        # 获取polarALL_state_int16当前步骤的状态并转换为复数
