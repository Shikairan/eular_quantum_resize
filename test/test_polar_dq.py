...
def compare_implementations_polar_dq(n_qubits=4, a=30, b=0, c=0):
    """
    ...
    # 3. 运行 polarALL_state_int16 并获取历史
    polar_final, polar_scale_final, polar_history = process_sequence_polar(
        initial_vec.copy(), seq, verbose=False
    )

    # 4. 比较初始状态
    dq_initial_converted = bit_order_conversion(initial_tensor.cpu().numpy(), n_qubits)
    polar_initial_complex = polar_to_complex_tensor(
        polar_history[0][0], polar_history[0][1]
    ).cpu().numpy().flatten()

    max_diff, rms_diff, mean_diff, error_percent = compute_state_difference(
        polar_initial_complex, dq_initial_converted
    )
...
if __name__ == "__main__":
    compare_implementations_polar_dq(n_qubits=14, a=80)
