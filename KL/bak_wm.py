...
def f(x, a=5.0):
    ...
    返回:
        计算结果，四舍五入到最接近的整数
    """
    return torch.round(254 * w(x, a))+1
...
def create_normalized_vector(size=10, seed=42, distribution='uniform', **kwargs):
    ...
    if distribution == 'uniform':
        ...
        x = torch.distributions.Exponential(rate).sample((size,))
    elif distribution == 'beta':
        # Beta分布
        alpha = kwargs.get('alpha', 1.0)
        beta = kwargs.get('beta', 1.0)
        x = torch.distributions.Beta(alpha, beta).sample((size,))
    elif distribution == 'gamma':
        # Gamma分布
        shape = kwargs.get('shape', 2.0)
        rate = kwargs.get('rate', 1.0)
        x = torch.distributions.Gamma(shape, rate).sample((size,))
    elif distribution == 'normal':
        # 正态分布（取绝对值）
        loc = kwargs.get('loc', 0.0)
        scale = kwargs.get('scale', 1.0)
        x = torch.abs(torch.distributions.Normal(loc, scale).sample((size,)))
    else:
...
def plot_kl_vs_a(X, a_range=None, filename="KL_A.jpg"):
    ...
    if a_range is None:
        ...
        a_range = torch.tensor(a_range, dtype=torch.float32)
...
if __name__ == "__main__":
    ...
    X_vectors = []
    labels = []
    a_range = torch.arange(3.9, 3.95, 0.00001)  # 恢复合理的范围和步长
    for dist_name, dist_params in distributions:
        print(f"\n--- {dist_name.upper()} DISTRIBUTION ---")
        X = create_normalized_vector(size=256, seed=seed, distribution=dist_name, **dist_params)
