...
5. 【新增】门操作时自动转换为 float 进行计算，避免 int8 量化误差累积

⚠️ 重要说明：
- 状态向量使用 PyTorch 张量，形状为 (N, 2)，其中 N=2^n 是状态数量
- 第一列存储幅度 r，第二列存储相位 theta
- 所有运算都在 GPU 上进行（如果可用）
- 门操作时自动在 int8 <-> float 之间转换，确保数值精度
"""
...
def hadamard_polar_tensor(z0_batch: torch.Tensor, z1_batch: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    自动处理 int8 <-> float 转换以避免量化误差
    z0_batch, z1_batch: 形状 (batch_size, 2) 的完整状态向量 [幅度, 相位]
    返回: 两个形状 (batch_size, 2) 的完整状态向量
    """
...
def z_polar_tensor(z0_batch: torch.Tensor, z1_batch: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    # 更新相位：相位翻转 π
    result_z1[:, 1] = encode_th_tensor(decode_th_tensor(z1_batch[:, 1].float()) + math.pi)
...
def s_polar_tensor(z0_batch: torch.Tensor, z1_batch: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    result_z1 = z1_batch.clone()
    # 更新相位：添加 π/2
    result_z1[:, 1] = encode_th_tensor(decode_th_tensor(z1_batch[:, 1].float()) + math.pi / 2)
...
def t_polar_tensor(z0_batch: torch.Tensor, z1_batch: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    result_z1 = z1_batch.clone()
    # 更新相位：添加 π/4
    result_z1[:, 1] = encode_th_tensor(decode_th_tensor(z1_batch[:, 1].float()) + math.pi / 4)
