...
def apply_controlled_gate_complex(vec: List[complex], control_idx: int, target_idx: int, gate_func, *gate_params):
    ...
    # 对于每个控制比特为 1 的状态，找到对应的目标比特为 0 和 1 的状态对
    # 我们需要遍历所有控制比特为 1 的状态对
    processed = set()
    for i in range(n):
        if (i & control_mask) != 0:  # 控制比特为 1
...
RESCALE_THRESHOLD = 0.3  # 可调：论文里用 1e-3~1e-4
...
# ---- 罇ª轀~B幾T读佅¥ ----
npy_files = list(Path('.').glob('error_cube.npy'))
if len(npy_files) != 1:
