"""
使用两个 int8 配合欧拉公式描述量子计算中的复数向量 - 改进版本

✨ 主要改进：
1. 渐进式缩放 (AdaptiveScaler)：避免大幅跳跃导致的量化误差
2. 误差补偿机制 (ErrorCompensator)：动态跟踪和补偿量化误差
3. 混合精度表示 (MixedPrecisionState)：对重要状态使用更高精度
4. 动态精度调整：基于误差分析的智能缩放策略
...
class AdaptiveScaler:
    """渐进式缩放器，避免大幅跳跃导致的量化误差"""
    def __init__(self, target_max=0.8, min_scale=0.1, max_scale_step=2.0):
        self.target_max = target_max
        self.min_scale = min_scale
        self.max_scale_step = max_scale_step  # 最大单步缩放倍数
        self.scale_accumulator = 1.0  # 累积缩放因子
        
    def should_scale(self, byte_vec):
        amps = [decode_r(b0) for b0, _ in byte_vec if decode_r(b0) > 0]
        if not amps:
            return False
        curr_max = max(amps)
        return curr_max < self.target_max * 0.5  # 早期干预
    
    def apply_adaptive_scale(self, byte_vec):
        """应用渐进式缩放"""
        if not self.should_scale(byte_vec):
            return
            
        # 计算当前最大振幅
        amps = [decode_r(b0) for b0, _ in byte_vec]
        curr_max = max(amps) if amps else 0
        
        if curr_max > 0 and curr_max < self.target_max:
            # 渐进式缩放，避免大幅跳跃
            scale_factor = min(self.target_max / curr_max, self.max_scale_step)
            scale_factor = max(scale_factor, self.min_scale)
...
class ErrorCompensator:
    ...
    def __init__(self, history_size=100):
        self.error_history = []  # 记录历史误差
        self.compensation_factors = {}  # 补偿因子
        self.history_size = history_size
        
    def track_error(self, original_complex, reconstructed_complex):
        """跟踪量化误差"""
        error = abs(original_complex - reconstructed_complex)
    ...
    def get_compensation_factor(self, gate_name, max_compensation=1.1):
        """获取补偿因子"""
        if gate_name not in self.compensation_factors:
            return 1.0
            
        # 基于最近误差计算补偿因子
        if self.error_history:
            base_error = np.mean(self.error_history[-min(10, len(self.error_history)):])
            compensation = 1.0 + base_error * self.compensation_factors[gate_name]
            return min(compensation, max_compensation)
        return 1.0
    
    def update_compensation(self, gate_name, observed_error, learning_rate=0.1):
        """更新补偿因子"""
        if gate_name not in self.compensation_factors:
            self.compensation_factors[gate_name] = 0.0
            
        # 指数移动平均更新
        current = self.compensation_factors[gate_name]
        self.compensation_factors[gate_name] = (
            learning_rate * observed_error + 
...
class MixedPrecisionState:
    ...
    def identify_important_states(self, byte_vec):
        """识别需要高精度的状态"""
        amplitudes = [(i, decode_r(b0)) for i, (b0, _) in enumerate(byte_vec)]
        ...
        for i in self.high_precision_indices:
            self.high_precision_amps[list(self.high_precision_indices).index(i)] = byte2complex(byte_vec[i])
...
# 全局误差处理实例
adaptive_scaler = AdaptiveScaler(target_max=0.8, min_scale=0.1, max_scale_step=2.0)
error_compensator = ErrorCompensator(history_size=100)

def improved_maybe_rescale_after_gate(name: str, byte_vec: List[Tuple[int, int]], scaler, compensator):
    """
    改进的门后缩放函数，结合渐进式缩放和误差补偿
    
    Args:
        name: 门名称
        byte_vec: 状态向量
        scaler: AdaptiveScaler实例
        compensator: ErrorCompensator实例
    """
    ...
    # 应用渐进式缩放
    scaler.apply_adaptive_scale(byte_vec)
    
    # 计算和跟踪量化误差
    total_error = 0
    for i, orig in enumerate(original_states):
        reconstructed = byte2complex(byte_vec[i])
        error = abs(orig - reconstructed)
        total_error += error
        compensator.track_error(orig, reconstructed)
    
    # 更新补偿因子
    avg_error = total_error / len(byte_vec)
    compensator.update_compensation(name, avg_error)
...
def apply_gate_unified(vec, byte_vec, gate_name, *gate_params, qubit_idx=0, control_idx=None, target_idx=None):
    ...
    if gate_name in controlled_gates:
        ...
        # 应用改进的缩放
        improved_maybe_rescale_after_gate(gate_name, byte_vec, adaptive_scaler, error_compensator)
...
np.save('error_cube_v2.npy', error_np)
