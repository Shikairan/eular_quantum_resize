...
# ------------ 复数 ground truth ------------
def h_complex_pair(z0: complex, z1: complex, theta: float):
    H = np.array([[1, 1], [1, -1]]) / math.sqrt(2.0)
    return H @ np.array([z0, z1])
# ------------ 终极修正版（直接抄） ------------
def h_polar_pair(z0: Tuple[int, int], z1: Tuple[int, int], theta: float):
    c0 = byte2complex(z0)
    c1 = byte2complex(z1)

    # H 矩阵乘：|0⟩→(|0⟩+|1⟩)/√2 , |1⟩→(|0⟩−|1⟩)/√2
    inv_sqrt2 = 1/math.sqrt(2)        # 1/√2
    c0p = inv_sqrt2 * (c0 + c1)
    c1p = inv_sqrt2 * (c0 - c1)
