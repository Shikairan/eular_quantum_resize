...
def hadamard_polar_tensor(z0_batch: torch.Tensor, z1_batch: torch.Tensor, scale_batch: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    c0 = vector.polar_to_complex_tensor(z0_batch, scale_batch)
    c1 = vector.polar_to_complex_tensor(z1_batch, scale_batch)
    ...
    # 分别转换为极坐标
    polar_result0, scale_result0 = vector.complex_to_polar_tensor(c0_result)
    polar_result1, scale_result1 = vector.complex_to_polar_tensor(c1_result)
...
def s_polar_tensor(z0_batch: torch.Tensor, z1_batch: torch.Tensor, scale_batch: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """PyTorch 版本：S 门极坐标实现（int16 + int8 混合版本，批量版本）
...
def t_polar_tensor(z0_batch: torch.Tensor, z1_batch: torch.Tensor, scale_batch: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """PyTorch 版本：T 门极坐标实现（int16 + int8 混合版本，批量版本）
...
def rz_polar_tensor(z0_batch: torch.Tensor, z1_batch: torch.Tensor, scale_batch: torch.Tensor, phi: float) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """PyTorch 版本：Rz 门极坐标实现（直接相位操作，int16 + int8 混合版本，批量版本）
...
def u1_polar_tensor(z0_batch: torch.Tensor, z1_batch: torch.Tensor, scale_batch: torch.Tensor, lambda_param: float) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """U1(λ) 只给 |1⟩ 添加相位 λ（与 polarALL_state_3 一致）"""
    result_z0 = z0_batch.clone()
    result_z1 = z1_batch.clone()
...
def u2_polar_tensor(z0_batch: torch.Tensor, z1_batch: torch.Tensor, scale_batch: torch.Tensor, phi: float, lambda_param: float) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """U2 门极坐标实现（与 polarALL_state_3 一致）"""
    c0 = vector.polar_to_complex_tensor(z0_batch, scale_batch)
    c1 = vector.polar_to_complex_tensor(z1_batch, scale_batch)
    dtype = c0.dtype
    device = c0.device
...
def u3_polar_tensor(z0_batch: torch.Tensor, z1_batch: torch.Tensor, scale_batch: torch.Tensor, theta: float, phi: float, lambda_param: float) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """U3 门极坐标实现（与 polarALL_state_3 一致）"""
    c0 = vector.polar_to_complex_tensor(z0_batch, scale_batch)
    c1 = vector.polar_to_complex_tensor(z1_batch, scale_batch)
    dtype = c0.dtype
    device = c0.device
...
def apply_gate_polar_tensor(polar_vec: torch.Tensor, scale_vec: torch.Tensor,
                          gate_name: str, gate_params: List[float] = None,
                          control_idx: int = None, target_idx: int = None) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    高级门应用函数，支持控制门（int16 + int8 混合版本）
    ...
    if gate_name in controlled_gates_no_param + controlled_gates_single_param + controlled_gates_multi_param:
        ...
        if gate_name == 'CNOT':
            ...
            apply_ch_polar_tensor(polar_vec, scale_vec, control_idx, target_idx)
        elif gate_name == 'CY':
            apply_cy_polar_tensor(polar_vec, scale_vec, control_idx, target_idx)
        elif gate_name == 'CS':
            apply_cs_polar_tensor(polar_vec, scale_vec, control_idx, target_idx)
        elif gate_name == 'CT':
            apply_ct_polar_tensor(polar_vec, scale_vec, control_idx, target_idx)
        elif gate_name == 'CRx':
            if not gate_params:
                raise ValueError(f"CRx 需要参数 theta")
            apply_crx_polar_tensor(polar_vec, scale_vec, control_idx, target_idx, gate_params[0])
        elif gate_name == 'CRy':
            if not gate_params:
                raise ValueError(f"CRy 需要参数 theta")
            apply_cry_polar_tensor(polar_vec, scale_vec, control_idx, target_idx, gate_params[0])
        elif gate_name == 'CRz':
            if not gate_params:
                raise ValueError(f"CRz 需要参数 phi")
            apply_crz_polar_tensor(polar_vec, scale_vec, control_idx, target_idx, gate_params[0])
        elif gate_name == 'CU1':
            if not gate_params:
                raise ValueError(f"CU1 需要参数 lambda")
            apply_cu1_polar_tensor(polar_vec, scale_vec, control_idx, target_idx, gate_params[0])
        elif gate_name == 'CU2':
            if len(gate_params) < 2:
                raise ValueError(f"CU2 需要参数 phi, lambda")
            apply_cu2_polar_tensor(polar_vec, scale_vec, control_idx, target_idx, gate_params[0], gate_params[1])
        elif gate_name == 'CU3':
            if len(gate_params) < 3:
                raise ValueError(f"CU3 需要参数 theta, phi, lambda")
            apply_cu3_polar_tensor(polar_vec, scale_vec, control_idx, target_idx, gate_params[0], gate_params[1], gate_params[2])

    else:
        # 单比特门
        polar_vec, scale_vec = apply_polar_gate_tensor(polar_vec, scale_vec, POLAR_GATES_TORCH[gate_name], *gate_params, qubit_idx=target_idx or 0)
...
def create_random_polar_state(n_amps: int) -> Tuple[torch.Tensor, torch.Tensor]:
    """创建随机极坐标 int16 + int8 状态向量"""
    return vector.create_random_state(n_amps)
...
def process_sequence_polar(initial_vec: List[complex], seq: List[Tuple], verbose: bool = False) -> Tuple[torch.Tensor, torch.Tensor, List[Tuple[torch.Tensor, torch.Tensor]]]:
    """
    ...
    Returns:
        (final_vec, final_scale_vec, state_history): 最终状态向量、缩放向量和每一步的状态历史
    """
    ...
    # 转换为极坐标格式
    vec, scale_vec = vector.complex_to_polar_tensor(complex_tensor)
    ...
    for step, gate_tuple in enumerate(seq):
        ...
        if verbose:
            ...
            if step < 2 or step % 10 == 0:  # 只显示前几个和每10个门的状态
                print(f"  状态（前8个）: {polar_vec_to_string(vec, scale_vec, range(min(8, n_amps)))}")

    if verbose:
        # 计算最终归一化
        complex_vec = vector.to_complex(vec, scale_vec)
...
def benchmark_polar_int16(n_qubits: int = 4, n_sequences: int = 100, sequence_length: int = 50) -> dict:
    ...
    for seq_idx in range(n_sequences):
        ...
        # 处理序列
        final_polar_vec, _ = process_sequence_polar(initial_vec, seq, verbose=False)
        results.append(final_polar_vec)
...
if __name__ == "__main__":
    ...
    print(f"Created random 2-qubit state with shape: {polar_vec.shape}")
    print(f"Data types: r={vector.amplitude_dtype} ({polar_vec[:, 0].dtype}), theta={vector.phase_dtype} ({polar_vec[:, 1].dtype})")
    print(f"State string: {polar_vec_to_string(polar_vec, scale_vec)}")

    # 测试量子门
    print("\nTesting quantum gates...")
    polar_vec, scale_vec = apply_polar_gate_tensor(polar_vec, scale_vec, POLAR_GATES_TORCH['H'], qubit_idx=0)
    print(f"After H gate: {polar_vec_to_string(polar_vec, scale_vec)}")

    polar_vec, scale_vec = apply_polar_gate_tensor(polar_vec, scale_vec, POLAR_GATES_TORCH['X'], qubit_idx=1)
    print(f"After X gate: {polar_vec_to_string(polar_vec, scale_vec)}")
    ...
    final_polar_vec, final_scale_vec, state_history = process_sequence_polar(initial_vec, seq, verbose=True)
...
def polar_to_complex_tensor(polar_vec: torch.Tensor, scale_vec: torch.Tensor) -> torch.Tensor:
    """
    兼容性函数：将极坐标向量转换为复数向量
    使用 PolarVector 类的 to_complex 方法
    """
    return vector.to_complex(polar_vec, scale_vec)
